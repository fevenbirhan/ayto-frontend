...
export const ReportForm = ({ onSubmitSuccess, reportToEdit, mode = 'create' }: ReportFormProps) => {
  ...
  return (
    <div className={`${mode === 'edit' ? 'fixed inset-0 z-50 overflow-y-auto bg-black/50 backdrop-blur-sm' : 'h-full'}`}>
      <div className={`${mode === 'edit' ? 'min-h-screen px-4 flex items-center justify-center' : 'h-full'}`}>
        <div className={`
          ${mode === 'edit' 
            ? 'relative z-50 w-full max-w-4xl my-8' 
            : 'h-[calc(100vh-8rem)] overflow-y-auto rounded-lg'
          }
        `}>
          <form onSubmit={handleSubmit} className="space-y-6 p-6 bg-[#1A1A1A] rounded-lg shadow-xl">
            ...
            <div className="flex justify-end space-x-4 pt-6 border-t border-[#404040] sticky bottom-0 bg-[#1A1A1A] py-4">
              <Button 
                type="button" 
                variant="outline" 
